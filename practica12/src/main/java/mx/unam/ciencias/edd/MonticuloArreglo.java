...
public class MonticuloArreglo<T extends ComparableIndexable<T>>
    ...
    public MonticuloArreglo(Iterable<T> iterable, int n) {
        ...
        for(T e : iterable) {
            arreglo[elementos] = e;
            arreglo[elementos].setIndice(elementos++);
        }
    }
    ...
    @Override public T elimina() {
        ...
        while(arreglo[minimo] == null)  // nos aseguramos de que tengamos un elemento distinto de null
        // para iniciar las comparaciones.
            minimo++;
        
        for(int i = minimo + 1; i < arreglo.length; i++){
            if(arreglo[i] == null) 
                continue;
            else if(arreglo[i].compareTo(arreglo[minimo]) < 0)
                minimo = i;
        }
        
        T eliminado = arreglo[minimo];
