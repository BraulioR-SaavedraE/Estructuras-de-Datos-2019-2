...
public class Grafica<T> implements Coleccion<T> {
    ...
    public void conecta(T a, T b) {
        ...
        primero.vecinos.agrega(b, new Vecino(segundo, 1));
        segundo.vecinos.agrega(a, new Vecino(primero, 1));
        aristas++;
    }
    ...
    public void conecta(T a, T b, double peso) {
        ...
        primero.vecinos.agrega(b, new Vecino(segundo, peso));
        segundo.vecinos.agrega(a, new Vecino(primero, peso));
        aristas++;
    }
     ...
     */
    private void eliminaVecino(Diccionario<T, Vecino> vecinos, T elemento){
        for(Vecino vertex :  vecinos)
            if(vertex.vecino.elemento.equals(elemento)){
                vecinos.elimina(elemento);
                return;
            }
    }
     ...
     */
    private Vecino busca(Diccionario<T, Vecino> vecinos, T elemento){
    ...
    public boolean esConexa() {
        // Aquí va su código.
        Diccionario<T, Vertice> diccionario = new Diccionario<T, Vertice>();
        Iterator<Vertice> recorre = vertices.iterator();

        bfs(recorre.next().get(), v -> diccionario.agrega(v.get(), (Vertice)v));

        return vertices.getElementos() == diccionario.getElementos();
    }
    ...
    @Override public boolean equals(Object objeto) {
        ...
        // Aquí va su código.
        if(getAristas() != grafica.getAristas() || vertices.getElementos() != grafica.vertices.getElementos())
            return false;
    ...
    public Lista<VerticeGrafica<T>> dijkstra(T origen, T destino) {
        ...
        Lista<VerticeGrafica<T>> miLista = new Lista<VerticeGrafica<T>>();
        Lista<Vertice> auxilio = new Lista<Vertice>();
        Iterator<Vertice> recorre = vertices.iterator();

        while(recorre.hasNext()) 
            auxilio.agregaFinal(recorre.next());
        
        if(getAristas() > ((getElementos() * (getElementos() -1)) / 2) -1)  // cuando la gráfica no es plana.
            mon = new MonticuloArreglo<Vertice>(auxilio);
        else
            mon = new MonticuloMinimo<Vertice>(auxilio);
